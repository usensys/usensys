...
The purpose of the code below is to acquire and preprocess US electric power sector data from EIA, EPA, and other sources. The final processed data, which is required to run USENSYS model, is stored in the '/data' directory in '.RData' format in the model working directory. It is not required to run scripts in this file to be able to run all the versions of USENSYS, available on on GitHub. The code below is provided for reference, and in order to be able to change the aggregation procedure.
