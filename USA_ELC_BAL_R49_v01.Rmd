...
if (T) { 
  ...
  trd_xl <- readxl::read_excel("data/trade_matrix01.xlsx",
                               range = "A1:AX50")
  # For bidirectional trade only, keep one direction
...
TRD_ELC_EXO <- newRepository(name = "TRD_ELC_EXO")
for (i in 1:dim(trd_dt)[1]) {
  src <- trd_dt$src[i]
  dst <- trd_dt$dst[i]
  trd_nm <- paste0("TRD_ELC_", src, "_", dst)  # Trade object name
  cmd_nm <- "ELC"
  ...
  TRD_ELC_EXO <- add(TRD_ELC_EXO, trd)
}
names(TRD_ELC_EXO@data)
# TRBD_UHV_NEI@data[[1]]
