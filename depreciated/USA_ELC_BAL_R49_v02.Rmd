...
<!--                  data = trd_map, inherit.aes = FALSE, size = 3,  -->
...
<!-- # trd_dt$cost <- round(trd_dt$losses * convert("USD/kWh", "MRMB/PJ", .06), 3) # Assuming 6 US cents per kWh for the losses -->
<!-- # trd_dt$cost <- round(trd_dt$distance_km / 1e3 *  -->
<!-- #                        convert("USD/kWh", "MUSD/GWh", .012), 3) # Assuming 1.2 US cents per kWh for the transmission -->
<!-- trd_dt$cost <- round(trd_dt$distance_km / 1e3 *  -->
<!--                        convert("USD/kWh", "MUSD/GWh", .001), 5) # Assuming 0.1 US cents per kWh per 1000 km for the transmission -->
...
<!--     cmd_nm <- "ELC" -->
<!--     cmd_nm_nbr <- "ELC" -->
<!--     # tch_src_nm <- paste0("TELC_", src_nm, "_SRC") # Technology name - transportation to source region border -->
...
# 3. Solving the model
# solve_model(tmp.dir = "solwork/RENBAL")
...
# 3. Solving the model
# solve_model(tmp.dir = "solwork/RENBAL4")
# Alterntively, run GAMS directly in the model folder: 'gams mdl'

# 4. Read the solution
# load("tmp/scen_REN4_interpolated.RData")
# scen_REN4 <- read_solution(scen_REN4, dir.result = "solwork/RENBAL4ZZ")
scen_REN4 <- read_solution(scen_REN4, dir.result = "solwork/RENBAL4ZZ60GW")
summary(scen_REN4)

save(scen_REN4, file = "scenarios/scen_RENBAL4ZZ60GW.RData")
...
(vTradeCap <- getData(scen_REN5, name = "vTradeCap", merge = T, year = max(msy),
                      newNames = c(value = "GW")))
...
(vTradeInv <- getData(scen_REN5, name = "vTradeInv", merge = T))
...
# Costs
```{r Costs}
sns <- list("GRID100" = scen_REN5, "GRID50" = scen_REN8, "GRID25" = scen_REN7)
...
inv_all_reg <- vInvAll %>% group_by(scenario, name) %>%
...
inv_all_reg$scenario <- factor(inv_all_reg$scenario, 
                               levels = c("GRID100", "GRID50", "GRID25"),
                               ordered = T)
