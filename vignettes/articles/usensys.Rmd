...
## A minimal example
The package currently includes an aggregated version of a 26-zone model developed for Model Intercomparison Project. Data for the original model will be published in separate repository. The simplified version has lower granularity but has the same structure. 

### Load data for the 26-zone model
```{r, eval = FALSE}
...
```{r, eval=FALSE}
scen_REF_test <- solve(scen_REF_test, wait = FALSE) 
scen_CAP_test <- solve(scen_CAP_test, wait = FALSE) 
# use wait = TRUE to run scenarios sequentially
```
